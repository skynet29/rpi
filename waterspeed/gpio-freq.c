...
static int gpio_freq_open (struct inode * ind, struct file * filp)
{
  ...
  //fout = fopen("/tmp/gpio_period.log", "w");
...
static int gpio_freq_read(struct file * filp, char * buffer, size_t length, loff_t * offset)
{
  ...
  struct timeval tv;
  unsigned long long t; // in ms
  unsigned static long long now = 0;
  ...
  t = (unsigned long long)tv.tv_sec*1000+tv.tv_usec/1000;
  if (now<0) now = t;
  
  snprintf(kbuffer, 128, "%d\n", data->frequency); // period in micro seconds is sent
  
  if (t - now < 100) {
     //snprintf(kbuffer, 128, "{\"tref\":%llu, \"period\":%d}\n", t, data->frequency); // period in micro seconds is sent
     // do nothing only send at 10 Hz
  }
  else {
    //snprintf(kbuffer, 128, "{\"tref\":%llu, \"period\":%d}\n", t, data->frequency); // period in micro seconds is sent
    //fputs(kbuffer, fout);
...
static irqreturn_t gpio_freq_handler(int irq, void * arg)
{
	...
	if ((data->last_timestamp.tv_sec  != 0)
	 || (data->last_timestamp.tv_usec != 0)) {
		...
		if (period > 0)
			data->frequency = period; // period is returned
		else
			data->frequency = 0;
